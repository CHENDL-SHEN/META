...
class ASPP_V2(nn.Module):
    def __init__(self, output_stride, norm_fn):
        ...
        self.aspp1 = ASPPModule(inplanes, 256, 3, padding=dilations[0], dilation=dilations[0], norm_fn=norm_fn)
        self.aspp2 = ASPPModule(inplanes, 256, 3, padding=dilations[1], dilation=dilations[1], norm_fn=norm_fn)
        self.aspp3 = ASPPModule(inplanes, 256, 3, padding=dilations[2], dilation=dilations[2], norm_fn=norm_fn)
        self.aspp4 = ASPPModule(inplanes, 256, 3, padding=dilations[3], dilation=dilations[3], norm_fn=norm_fn)
        ...
        self.initialize([self.conv1, self.bn1])
    
    def forward(self, x):
        x1 = self.aspp1(x)      # 16,256,32,32
        x2 = self.aspp2(x)      # 16,256,32,32
        x3 = self.aspp3(x)      # 16,256,32,32
        x4 = self.aspp4(x)      # 16,256,32,32
 
        x = torch.cat((x1, x2, x3, x4), dim=1)    # 16,1024,32,32
