...
def save_QIMG(images):
    ...
    for i in range(16):
        ...
        plt.imsave(f'/media/ders/XS/SPCAM/experiments/TEST/IMG1/image_{i}.png', final_array)

def save_IPC_estimate_IMG(image, fg_cam_, id):
    import numpy as np
    import matplotlib.pyplot as plt
    ...
    norm_image = np.empty_like(np.asarray(image), np.float32)

    norm_image[..., 0] = (image[..., 0] / 255. - imagenet_mean[0]) / imagenet_std[0]
    norm_image[..., 1] = (image[..., 1] / 255. - imagenet_mean[1]) / imagenet_std[1]
    norm_image[..., 2] = (image[..., 2] / 255. - imagenet_mean[2]) / imagenet_std[2]

    sailencys = torch.from_numpy(norm_image.transpose((2, 0, 1))).unsqueeze(0).cuda().float()  # sailencys：（1, 3, 400，500）

    fg_cam = fg_cam_[0][1:].unsqueeze(0)
    b, c, h, w = fg_cam.size()                  # fg_cam:(1,20,400,500) 
    imgmin_mask = sailencys.sum(1, True) != 0   # sailencys:(1,3,400,500);imgmin_mask:(1,1,400,500);bool; sailencys.sum(1, True):1是sum第一个维度，True是sum后保持维度不变; != 是将sailencys.sum(1, True) 的结果变成布尔量
    # sailencys = F.interpolate(sailencys.float(), size=(h, w))   # sailencys:(1,3,400,500)

    bg = 1-torch.max(fg_cam, dim=1, keepdim=True)[0] ** 1       # bg:(1,1,400,500)

    nnn = torch.max((1 - bg.detach() * imgmin_mask).view(b, 1, -1), dim=2)[0] > 0.1   # nnn:(144,1);bool; self.args.ig_th=0.1
    nnn2 = torch.max((bg.detach() * imgmin_mask).view(b, 1, -1), dim=2)[0] > 0.1      # nnn2:(144,1);bool
    ...
    probs = torch.cat([bg, fg_cam], dim=1)              # probs:(1,21,400,500)
    probs1 = probs * imgmin_mask                        # probs1:(1,21,400,500)

    origin_f = F.normalize(sailencys.detach(), dim=1)   # origin_f:(1,3,400,500)
    origin_f = origin_f * imgmin_mask                   # origin_f:(1,3,400,500)

    f_min = pool_feat_2(probs1, origin_f)               # f_min(1,3,21)
    up_f = up_feat_2(probs1, f_min)                     # up_f(1,3,400,500)

    # 将Tensor转换为numpy数组
    numpy_array = up_f.cpu().detach().numpy()
    # 调整数组的形状以适应图像显示
    reshaped_array = np.transpose(numpy_array, (0, 2, 3, 1))
    ...
    # 转换数组的数据类型为uint8
    final_array = clipped_array.astype(np.uint8)
    plt.imsave(f'/media/ders/XS/SPCAM/experiments/TEST/IMG/image_IPC_%s.png' % id, final_array)
...
def init_spixel_grid(args,  b_train=True):
    ...
    torch_spix_idx_tensor = torch.from_numpy(np.tile(spix_idx_tensor, (batch_size, 1, 1, 1))).type(torch.float).cuda()
    ...
    all_XY_feat = (torch.from_numpy(np.tile(coord_tensor, (batch_size, 1, 1, 1)).astype(np.float32)).cuda())
